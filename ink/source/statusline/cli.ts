...
function parseFlags(argv: string[]): CliFlags {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--preview") flags.preview = true;
    else if (a === "--list") flags.list = true;
    else if (a === "--list-packs") flags.listPacks = true;
    else if (a === "--list-palettes") flags.listPalettes = true;
    else if (a === "--explain") flags.explain = true;
    else if (a === "--debug") flags.debug = true;
    else if (a === "--install-fonts") flags.installFonts = true;
    else if (a === "--konami") flags.konami = true;
    else if (a === "--version" || a === "-v") flags.version = true;
    else if (a === "--help" || a === "-h") flags.help = true;
    else if (a === "--no-git") flags.noGit = true;
    else if (a === "--corrupt") flags.corrupt = true;
    ...
    else if (a === "--scenes") flags.scenes = (next() ?? "").split(",");
    else if (a.startsWith("--exclude=")) flags.exclude = a.slice(10).split(",");
    else if (a === "--exclude") flags.exclude = (next() ?? "").split(",");
    else if (a.startsWith("--palette=")) flags.palette = a.slice(10);
    else if (a === "--palette") flags.palette = next();
    else if (a.startsWith("--events=")) flags.events = a.slice(9).split(",");
    else if (a === "--events") flags.events = (next() ?? "").split(",");
    else if (a.startsWith("--cycle-seconds=")) flags.cycleSeconds = parseInt(a.slice(16), 10);
    else if (a === "--cycle-seconds") flags.cycleSeconds = parseInt(next() ?? "20", 10);
    else if (a.startsWith("--scene=")) flags.scene = a.slice(8);
    else if (a === "--scene") flags.scene = next();
    else if (a.startsWith("--effect=")) flags.effect = a.slice(9);
    else if (a === "--effect") flags.effect = next();
    else if (a.startsWith("--width=")) flags.width = parseInt(a.slice(8), 10);
    else if (a === "--width") flags.width = parseInt(next() ?? "80", 10);
  }
...
run().catch((err) => {
  process.stderr.write(`hakcer-statusline error: ${(err as Error).message}\n`);
