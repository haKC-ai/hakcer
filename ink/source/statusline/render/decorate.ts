...
interface Pair {
  left: { nerd: string; fallback: string };
  right: { nerd: string; fallback: string };
}

const G = (nerd: string, fallback: string) => ({ nerd, fallback });

// ─── verb → glyph pair (case-insensitive) — covers all 128 unique verbs ──
const VERB_DECOR: Record<string, Pair> = {
  // data / session
...
const PACK_DECOR: Record<string, Pair> = {
...
const DEFAULT_DECOR: Pair = {
  left: G("\uf120", ">_"), // terminal
  right: G("\uf0e7", "!"), // bolt
};

export function decoratorFor(scene: Scene): Pair {
  const verbKey = (scene.verb ?? "").toUpperCase();
  if (VERB_DECOR[verbKey]) return VERB_DECOR[verbKey]!;
  if (PACK_DECOR[scene.pack]) return PACK_DECOR[scene.pack]!;
  return DEFAULT_DECOR;
}

export function decorate(text: string, scene: Scene, caps: FontCaps): string {
