...
export function lookupModel(modelId: string): ModelPricing {
  // exact match
  if (MODELS[modelId]) return MODELS[modelId]!;
  // prefix match (strip date suffixes)
  for (const [key, val] of Object.entries(MODELS)) {
    if (modelId.startsWith(key.replace(/-\d{8}$/, ""))) return val;
  }
  // family match
  if (modelId.includes("opus")) return MODELS["claude-opus-4-6"]!;
  if (modelId.includes("sonnet")) return MODELS["claude-sonnet-4-6"]!;
  if (modelId.includes("haiku")) return MODELS["claude-haiku-4-5-20251001"]!;
  return FALLBACK;
}

export function calcCost(modelId: string, tokens: TokenUsage): CostInfo {
...
export function calcContext(modelId: string, tokens: TokenUsage): ContextInfo {
