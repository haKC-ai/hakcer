...
export async function installFonts(): Promise<number> {
  ...
  if (plat === "darwin") return installMacOS();
  if (plat === "linux") return installLinux();
...
async function installLinux(): Promise<number> {
