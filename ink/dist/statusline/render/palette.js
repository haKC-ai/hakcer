...
export function getPalette(id, library) {
...
export function buildPaintFns(palette) {
...
export function gradientLine(text, stops) {
    if (!text)
        return "";
    const cleaned = stripAnsi(text);
    const stopRgbs = stops.map((s) => hexToRgb(normalizeHex(s)));
    if (stopRgbs.length === 0)
        return text;
    const out = [];
