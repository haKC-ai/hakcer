...
export function readSecKCCache() {
    try {
...
export function getNextSecKCEvent(now = Date.now()) {
    ...
    const days = Math.floor((startOfDay(next._date).getTime() - startOfDay(nowDate).getTime()) / (1000 * 60 * 60 * 24));
...
export function refreshInBackground() {
    try {
        ...
        const child = spawn(process.execPath, ["-e", buildRefreshScript()], { detached: true, stdio: "ignore" });
        child.unref();
    }
    catch {
        // ignore
