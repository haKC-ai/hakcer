...
export function buildPool(library, opts) {
    ...
    pool = pool.filter((s) => {
        if (s.id.startsWith("events_seckc_")) {
            if (!opts.events?.includes("seckc"))
                return false;
            const ev = opts.data?.events?.seckc?.next;
            if (!ev)
                return false;
            if (s.id === "events_seckc_today")
                return ev.days_until === 0;
            if (s.id === "events_seckc_upcoming")
                return ev.days_until > 0 && ev.days_until <= 7;
        }
        // Trigger-only scenes are not in normal rotation
        if (TRIGGER_ONLY.has(s.id))
            return false;
        return true;
    });
    return pool;
