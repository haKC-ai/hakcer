...
export async function main(flags) {
    ...
    // List commands
    if (flags.list)
        return doList(library, flags);
    if (flags.listPacks)
        return doListPacks(library);
    if (flags.listPalettes)
        return doListPalettes(library);
    // Preview loop — render repeatedly with wall-clock tick advancing
    if (flags.preview)
        return runPreview(library, flags);
    ...
    const screensaverOptIn = process.env.HAKCER_STATUSLINE_SCREENSAVER === "1" || flags.screensaver === true;
    ...
    if (!flags.konamiOff && konamiActive(now)) {
        ...
        if (konami) {
            const konamiRender = interpretScene({ ...konami, effect: "rainbow" }, renderCtx, library);
...
async function runPreview(library, flags) {
    const width = flags.width ?? detectWidth();
    const fontCaps = detectFontCaps();
...
function doListPalettes(library) {
