...
export function rgbToAnsi(rgb) {
    return `\x1b[38;2;${rgb.r};${rgb.g};${rgb.b}m`;
}
export function resetAnsi() {
    return "\x1b[0m";
}
export function colorize(char, hex) {
    if (char === " " || char === "\n")
        return char;
 ...
 */
export function gradient(stops, steps) {
    if (steps <= 0)
        return [];
    if (steps === 1)
        return [stops[0]];
    if (stops.length === 1)
        return Array(steps).fill(stops[0]);
    const rgbStops = stops.map(hexToRgb);
    const result = [];
    const segmentSize = (steps - 1) / (rgbStops.length - 1);
    for (let i = 0; i < steps; i++) {
        const segment = Math.min(Math.floor(i / segmentSize), rgbStops.length - 2);
 ...
 */
export function applyGradient(text, colors) {
    const chars = [...text];
